...
-(void)loadWebView
{
    ...
    webView.tag = 121;
    //NSURL *targetURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://108.178.27.242/luigi/webs/printable/%@",[ShareableData sharedInstance].assignedTable1]];
    //NSURLRequest *request = [NSURLRequest requestWithURL:targetURL];
   // [webView  loadRequest:request];
   // [self.view addSubview:webView];
    
    NSURL *targetURL2 = [NSURL URLWithString:[NSString stringWithFormat:@"%@Ex/printHTMLBill.php?tableid=%@&key=%@", [ShareableData serverURL],[ShareableData sharedInstance].assignedTable1, [ShareableData appKey]]];
   ...
   /* for(int i=0;i<[[ShareableData sharedInstance].TaxNameValue count];i++)
    {
        ...
        {
            ////NSLOG(@"%@",[[ShareableData sharedInstance].TaxList objectAtIndex:i]);
            amountafterdiscount+=cv;
        }
    }
    
    amountafterdiscount=t-amountafterdiscount;
    //NSLOG(@"Amount After Discount : %f",amountafterdiscount);
    ...
    totalAmount = [NSString stringWithFormat:@"%@", [self getTotal]];
    ...
    /*====================================Adding Payment View==================================*/
    CGRect frm = CGRectMake(0, 38, 712, 340);
    paymentView = [[PaymentModeView alloc] initWithFrame:frm totalAmount:totalAmount];
    [self.v1 addSubview:paymentView];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self
                                                                          action:@selector(dismissPickerVieww)];
    
    [self.view addGestureRecognizer:tap];
    
}

-(void)dismissPickerVieww{
    [paymentView dismissPicker];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView==TaxesTable)
    {
        ...
        TaxName.text= [[ShareableData sharedInstance].TaxList objectAtIndex:indexPath.row];;
        ...
        TaxValue.text= [NSString stringWithFormat:@"$%@",[TaxListValue objectAtIndex:indexPath.row]];
    ...
    else
    {
        ...
        if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"1"])
            ItemName.text= [[ShareableData sharedInstance].OrderItemName objectAtIndex:indexPath.row];
        else
            ItemName.text= [[ShareableData sharedInstance].DishName objectAtIndex:indexPath.row];
        ...
        if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"1"])
            ItemQuantity.text= [[ShareableData sharedInstance].OrderItemQuantity objectAtIndex:indexPath.row];
        else
        ...
        if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"1"])
            ...
            ItemRate.text=[NSString stringWithFormat:@"$%@",[[ShareableData sharedInstance].DishRate objectAtIndex:indexPath.row]];
...
{
   BOOL isValid =  [paymentView isValidAmount];
    //[paymentView paymentNames]; This gives list of all string payment methods
    //[paymentView paymentAmounts]; This gives all amount corresponding to paymentNames
   ...
   /* if([x1 floatValue]==0 && [x2 floatValue]==0 && [x3 floatValue]==0 && [x4 floatValue]==0 && [x5 floatValue]==0 && [x6 floatValue]==0 && [x7 floatValue]==0 && [x8 floatValue]==0 && [x9 floatValue]==0 && [x10 floatValue]==0 && [x11 floatValue]==0 && [x12 floatValue]==0 && [x13 floatValue]==0)
    {
        ...
        [alert3 show];
    }else*/ if (!isValid){//([x19 floatValue] < SubTotal.text.floatValue){
...
-(NSString*)getTotal{
    ...
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@Ex/getTotalAmount.php?tableid=%@&key=%@", [ShareableData serverURL],tableNumber, [ShareableData appKey]]]];
    NSError *error;
...
-(NSString*)getDiscount{
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@Ex/getDiscountValue.php?tableid=%@&key=%@", [ShareableData serverURL],tableNumber, [ShareableData appKey]]]];
    NSError *error;
    NSURLResponse *response;
    NSData *uData=[NSURLConnection sendSynchronousRequest:request returningResponse:&response error:&error];
    NSString *data=[[[NSString alloc]initWithData:uData encoding:NSUTF8StringEncoding]stringByTrimmingCharactersInSet:
...
{
    NSString *post =[NSString stringWithFormat:@"table_id=%@&payment_name=%@&amount=%@&key=%@",orderId,PaymentMethodName,Amount, [ShareableData appKey]];
    ...
    NSString *url_string = [NSString stringWithFormat:@"%@/webs/set_payment", [ShareableData serverURL]];
...
{
    NSString *post =[NSString stringWithFormat:@"table_id=%@&dish_id=%@&dish_name=%@&quantity=%@&price=%@&tempOrderId=%@&isOrderCustomisation=%@&orderCatId=%@&orderSpecialRequest=%@&key=%@",orderId,ItemId,ItemName,Quantity,Rate,tempid,isOrderCust,orderCat,specialrequest, [ShareableData appKey]];
    ...
    NSString *url_string = [NSString stringWithFormat:@"%@/webs/set_order_items", [ShareableData serverURL]];
...
{
    NSString *post =[NSString stringWithFormat:@"charge_name=%@&value=%@&table_id=%@&disc=%@&key=%@",ChargeName,Amount,orderId,disc, [ShareableData appKey]];
    ...
    NSString *url_string = [NSString stringWithFormat:@"%@/webs/set_charges", [ShareableData serverURL]];
    [request setURL:[NSURL URLWithString:url_string]];
...
-(void)sendOther:(NSString*)orderId Pax:(NSString*)Pax Total1:(NSString*)Total1
{
    ...
    if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"1"])
        ...
        type=@"in";
    NSString *post =[NSString stringWithFormat:@"order_type=%@&pax=%@&total_amount=%@&table_id=%@&customer_id=%@&key=%@",type,Pax,Total1,orderId,[ShareableData sharedInstance].isLogin, [ShareableData appKey]];
    ...
    NSString *url_string = [NSString stringWithFormat:@"%@/webs/order_update", [ShareableData serverURL]];
    [request setURL:[NSURL URLWithString:url_string]];
...
- (void)myTask
{
    if(taskType==1)
    {
      ...
      //  NSString *URLEncodedText = [@"readPayment://" strin :NSUTF8StringEncoding];
        NSString *ourPath = [NSString stringWithFormat:@"sccp-swiff://pay?amount=%@&ref_1=%@&ref_2=%@&return=%@",totalAmount,BillNumber1,tableNumber,@"readPayment%3A%2F%2F"];
        //  DLog(@"%@",ourPath);
        ...
        if ([ourApplication canOpenURL:ourURL]) {
            ...
            //Display error
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Receiver Not Found" message:@"The SWIFF App is not installed. It must be installed to accept e-Payments." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alertView show];
        ...
        /*
         ...
         // if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"1"])
         //   tableNumber=@"1234";
         for (int i=0;i<[[paymentView paymentNames]count];i++){
         //[paymentView paymentNames]; This gives list of all string payment methods
         //[paymentView paymentAmounts];
         [self sendPayment:tableNumber PaymentMethodName:(NSString*)[[paymentView paymentNames] objectAtIndex:i] Amount:(NSString*)[[paymentView paymentAmounts] objectAtIndex:i]];
         }
         ...
         [self sendCharges:tableNumber ChargeName:[[ShareableData sharedInstance].TaxList objectAtIndex:i] Amount:[[ShareableData sharedInstance].TaxNameValue objectAtIndex:i ] disc:ttt];
         }
         ...
         [self sendDishes:tableNumber ItemId:[[ShareableData sharedInstance].OrderItemID objectAtIndex:i] ItemName:[[ShareableData sharedInstance].OrderItemName objectAtIndex:i] Rate:[[ShareableData sharedInstance].OrderItemRate objectAtIndex:i] Quantity:[[ShareableData sharedInstance].OrderItemQuantity objectAtIndex:i] tempid:[[ShareableData sharedInstance].TempOrderID objectAtIndex:i] isOrderCust:[[ShareableData sharedInstance].IsOrderCustomization objectAtIndex:i] orderCat:[[ShareableData sharedInstance].OrderCatId objectAtIndex:i] specialrequest:[[ShareableData sharedInstance].OrderSpecialRequest objectAtIndex:i]];
         ...
         [self sendDishes:tableNumber ItemId:[[ShareableData sharedInstance].DishId objectAtIndex:i] ItemName:[[ShareableData sharedInstance].OrderItemName objectAtIndex:i] Rate:[[ShareableData sharedInstance].DishRate objectAtIndex:i] Quantity:[[ShareableData sharedInstance].OrderItemQuantity objectAtIndex:i] tempid:[[ShareableData sharedInstance].TempOrderID objectAtIndex:i] isOrderCust:[[ShareableData sharedInstance].IsOrderCustomization objectAtIndex:i] orderCat:[[ShareableData sharedInstance].OrderCatId objectAtIndex:i] specialrequest:[[ShareableData sharedInstance].OrderSpecialRequest objectAtIndex:i]];
         }
         }
...
- (void)confirmPayment
{
    if(taskType==1)
    {
        ...
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
        [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://54.251.99.174/kinaraEx/printFullBillText.php?orderid=%@&tableid=%@",[ShareableData sharedInstance].OrderId,tableNumber]]];
...
-(IBAction)PrintBill:(id)sender
{
    ...
    [self.navigationController pushViewController:billprintView animated:YES];
...
-(void)printhere
{
   ...
   /*NSString *test1=@"kalim";
    ...
    NSData    *imageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/webs/printable/%@", [ShareableData serverURL],[ShareableData sharedInstance].assignedTable1]]];
    
    NSString *url_string = [NSString stringWithFormat:@"%@/webs/printable/2", [ShareableData serverURL]];
...
-(void)getTableTaxDetails
{
   ...
   // //NSLOG(@"Data :%@",data);
    ...
    ////NSLOG(@"Data :%@",resultFromPost);
    for(int i=0;i<[resultFromPost count];i++)
    {
        NSMutableDictionary *dataitem=[resultFromPost objectAtIndex:i];
        [[ShareableData sharedInstance].TaxList addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"charge_name"]]];
        [[ShareableData sharedInstance].TaxNameValue addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"value"]]];
        [[ShareableData sharedInstance].inFormat addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"in_format"]]];
        [[ShareableData sharedInstance].isDeduction addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"is_deduction"]]];
    }
}
...
-(void)getTaxesList
{
    if([[ShareableData sharedInstance].isTakeaway isEqualToString:@"0"])
    {
        ...
        {
            NSMutableDictionary *dataitem=[resultFromPost objectAtIndex:i];
            [[ShareableData sharedInstance].TaxList addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"name"]]];
            [[ShareableData sharedInstance].TaxNameValue addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"value"]]];
            [[ShareableData sharedInstance].inFormat addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"is_percent"]]];
            [[ShareableData sharedInstance].isDeduction addObject:[NSString stringWithFormat:@"%@",[dataitem objectForKey:@"is_deduction"]]];
        }
