...
@synthesize search,feedback,favourite,help,orderSummaryView;
...
- (void)viewDidLoad
{
    ...
    if(fontDictionary != nil) {
        ...
        fontSize = 30;
        fontColor = [UIColor whiteColor];//[UIColor colorWithRed:96.0/255.0 green:37.0/255.0 blue:36.0/255.0 alpha:1.0];
...
-(void)subCatAction:(id)sender
{
    ...
    for(int i = ([subs count]/2)-2; i < [subs count]; i++)
    {
        ...
        if([sub_view isMemberOfClass:[UIButton class]]) {
            ...
            if([[[_temp_btn titleForState:UIControlStateNormal] lowercaseString] isEqualToString:[[tmp titleForState:UIControlStateNormal] lowercaseString]]) {
                btn = tmp;
                break;
            }
        }
...
{
    /*
    UIImageView *bg_image_view = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, overviewMenuView.frame.size.width, 835)];
    [bg_image_view setImage:[UIImage imageNamed:@"menu_plane_bg.png"]];
    */
    if (menuTable) {
    ...
    //[mainMenu addSubview:bg_image_view];
...
{
	 NSString* CellIdentifier = [NSString stringWithFormat:@"MessageCellIdentifier %d",indexPath.row];
 ...
 //   NSString *img_name1 = [NSString stringWithFormat:@"%@%@", PRE_NAME, CATEGORY_IMAGE];
    NSString *img_name1 = [NSString stringWithFormat:@"%@%@_%@.png", PRE_NAME, CATEGORY_IMAGE, [ShareableData appKey]];
    UIImage *img1 = [[TabSquareDBFile sharedDatabase] getImage:img_name1];
    if(img1 == nil)
       // img1 = [UIImage imageNamed:@"cell_slide.png"];
        img1 =[UIImage imageWithContentsOfFile:@"cell_slide.png"];

    
    UIImageView *background = [[UIImageView alloc] initWithImage:img1];
    [background setFrame:CGRectMake(0, 0, menuTable.frame.size.width, 110.0)];
    [cell.contentView addSubview:background];
    
    
    /*=========================Category Name=========================*/
    UILabel *category_lbl = [[UILabel alloc] initWithFrame:CGRectMake(145.0, 35.0, 460.0, 50.0)];
    [category_lbl setFont:[UIFont fontWithName:fontName size:fontSize]];
    NSString *str = [categoryList objectAtIndex:indexPath.row];
    ...
    CGRect img_frm = CGRectMake(background.frame.origin.x+10, 0, 100, 93);
